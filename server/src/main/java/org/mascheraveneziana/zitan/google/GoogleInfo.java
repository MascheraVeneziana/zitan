...
public class GoogleInfo {
    ...
    private static GoogleInfo instance;

    public static GoogleInfo getInstance() throws IOException {
        if (instance == null) {
            instance = new GoogleInfo();
        }
        return instance;
    }
    ...
    private GoogleInfo() throws IOException {
        Properties prop = new Properties();
        try (
            InputStream in = GoogleInfo.class.getClassLoader().getResourceAsStream("application.properties");
        ) {
            prop.load(in);
        }
