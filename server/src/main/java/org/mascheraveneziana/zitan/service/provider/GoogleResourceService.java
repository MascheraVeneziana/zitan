...
public class GoogleResourceService implements ProviderResourceService {
    ...
    public List<ProviderResource> getResources(OAuth2AuthenticationToken authentication) throws Exception {
        ...
        List<ProviderResource> resourceList = calendarResources.getItems()
                .stream().map(item -> {
    ...
    public ProviderResource getResourceById(OAuth2AuthenticationToken authentication, String id) throws Exception {
        ...
        CalendarResource calendarResource = directoryService.resources().calendars().get("my_customer", id).execute();
